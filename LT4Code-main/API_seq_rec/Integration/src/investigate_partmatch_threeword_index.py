...
def check_matching_lines(txt_lines, test_lines):    #txt_lines:予測結果のAPIメソッドのリスト["Utility . getValidChoice ... Scanner . nextLine", "Error . <init> ... ExceptionInInitializer . error", ...], test_lines:テストデータのAPIメソッドのリスト["HashSet . <init> ... String . charAt", ]
    ...
    for i, (txt_apimethod, test_apimethod) in enumerate(zip(all_txt_apimethod_list, all_test_apimethod_list)):
        ...
        if (min_len == 3):  # 両方のAPIメソッドが2つ以上のAPIメソッドから構成されている場合
            if txt_apimethod[0] == test_apimethod[0] and txt_apimethod[1] == test_apimethod[1] and txt_apimethod[2] == test_apimethod[2]: # 3つ目のAPIメソッドまで一致している場合
                matching_indices.append(i+1)
        elif min_len >= 4:
            if txt_apimethod[0] == test_apimethod[0] and txt_apimethod[1] == test_apimethod[1] and txt_apimethod[2] == test_apimethod[2] and txt_apimethod[3] != test_apimethod[3]: # 3つ目のAPIメソッドまでが一致しているが4つ目のAPIメソッドが一致していない場合
                matching_indices.append(i+1)
...
def main():
    ...
    parser.add_argument('--output_file', type=str, default='../data/CodeT5_partmatch_threeword_index.txt', help="Path to save the output file")
