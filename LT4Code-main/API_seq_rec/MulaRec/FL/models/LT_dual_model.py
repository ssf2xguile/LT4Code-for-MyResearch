...
def balanced_softmax_loss(labels, logits, sample_per_class, reduction):
    ...
    loss = F.cross_entropy(input=logits, target=labels, reduction=reduction, ignore_index=-1)
    return loss
...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, \
        ...
        if self.fusion_flag == True:
            self.fusion1 = nn.Linear(config.hidden_size * 2, config.hidden_size)
            self.fusion2 = nn.Linear(config.hidden_size * 2, config.hidden_size)
            self.fusion3 = nn.Linear(config.hidden_size * 2, config.hidden_size)
    ...
    def forward(self,
                ...
                target_ids=None, target_mask=None, loss_flag='ce', samples_per_cls=None):
        ...
        if target_ids is not None:
            ...
            if LT_solution == 'ce':
                ...
                input_tensor = shift_logits.view(-1, shift_logits.size(-1))[active_loss]
                target_tensor = shift_labels.view(-1)[active_loss]
                ...
                input_tensor = shift_logits.view(-1, shift_logits.size(-1))[active_loss]
                target_tensor = shift_labels.view(-1)[active_loss]
                b_soft_loss = balanced_softmax_loss(target_tensor, input_tensor, samples_per_cls, 'mean')
                loss = b_soft_loss
...
class MulaRecDual():
    def __init__(self, codebert_path, decoder_layers, fix_encoder, beam_size, \
                 max_source_length, max_target_length, load_model_path, l2_norm, fusion, multi_gpu=False):
        ...
        logging.info("Multiple GPU training : {}".format(multi_gpu))
    ...
    def train(self,
              ...
              eval_batch_size, output_dir, gradient_accumulation_steps=1, loss_flag='ce', samples_per_cls=None):
        ...
        train_features = convert_examples_to_features(train_examples, self.tokenizer, self.max_source_length,
                                                      self.max_target_length, stage='train', debug=False)
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                loss, _, _ = self.model(
                    ...
                    loss_flag=loss_flag, samples_per_cls=samples_per_cls)
            ...
            if do_eval == True:
                ...
                for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                    ...
                    src_ant_ids, src_ant_mask, \
                    src_code_ids, src_code_mask = batch
    ...
    def predict(self, src_code, src_ant):
