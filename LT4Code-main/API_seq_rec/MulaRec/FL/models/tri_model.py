...
class Seq2Seq(nn.Module):
    ...
    def forward(self, \
        ...
        src_code_ids=None, src_code_mask=None, \
        sim_post_ids=None, sim_post_mask=None, \
        ...
        src_code_outputs = self.encoder(src_code_ids, attention_mask=src_code_mask)
        src_code_encoder_output = src_code_outputs[0].permute([1, 0, 2]).contiguous()
        
        sim_post_outputs = self.encoder(sim_post_ids, attention_mask=sim_post_mask)
        sim_post_encoder_output = sim_post_outputs[0].permute([1, 0, 2]).contiguous()
        ...
        if self.l2_norm == True:            
            ...
            src_code_encoder_output = F.normalize(src_code_encoder_output, p=2, dim=-1)
            sim_post_encoder_output = F.normalize(sim_post_encoder_output, p=2, dim=-1)
            
        if self.fusion_flag == True:
            output_1 = self.fusion1(
                torch.cat([
                    src_ant_encoder_output, \
                    src_code_encoder_output, \
                    sim_post_encoder_output
                ], dim=-1))
        ...
        if target_ids is not None:
            ...
            for i in range(src_code_ids.shape[0]):
                context = encoder_output[:, i:i + 1]
                context_mask = src_code_mask[i:i + 1, :]
...
class MulaRecTri():
    ...
    def train(self,
        ...
        all_src_code_ids = torch.tensor([f.src_code_ids for f in train_features], dtype=torch.long)
        all_src_code_mask = torch.tensor([f.src_code_mask for f in train_features], dtype=torch.long)
        
        all_sim_post_ids = torch.tensor([f.sim_post_ids for f in train_features], dtype=torch.long)
        all_sim_post_mask = torch.tensor([f.sim_post_mask for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(            
            ...
            all_src_code_ids, all_src_code_mask, \
            all_sim_post_ids, all_sim_post_mask, \
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                src_code_ids, src_code_mask, \
                sim_post_ids, sim_post_mask, \
                ...
                loss, _, _ = self.model(                    
                    ...
                    sim_post_ids=sim_post_ids, \
                    sim_post_mask=sim_post_mask, \
                ...
                nb_tr_examples += src_code_ids.size(0)
                nb_tr_steps += 1
                loss.backward()
            ...
            if do_eval==True:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    all_sim_post_ids = torch.tensor([f.sim_post_ids for f in eval_features], dtype=torch.long)
                    all_sim_post_mask = torch.tensor([f.sim_post_mask for f in eval_features], dtype=torch.long)
                ...
                for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                    ...
                    src_ant_ids, src_ant_mask, \
                    src_code_ids, src_code_mask, \
                    sim_post_ids, sim_post_mask = batch
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    preds = self.model(
                        ...
                        src_code_ids=src_code_ids, src_code_mask=src_code_mask, \
                        sim_post_ids=sim_post_ids, sim_post_mask=sim_post_mask
                    )
    ...
    def predict(self, src_ant, src_code, sim_post):        
        ...
        encode = self.tokenizer.encode_plus(src_code, return_tensors="pt", \
            max_length=self.max_source_length, truncation=True, \
                pad_to_max_length=True)        
        src_code_ids = encode['input_ids'].to(self.device)
        src_code_mask = encode['attention_mask'].to(self.device)
        
        encode = self.tokenizer.encode_plus(sim_post, return_tensors="pt", \
            max_length=self.max_source_length, truncation=True, pad_to_max_length=True)
        sim_post_ids = encode['input_ids'].to(self.device)
        sim_post_mask = encode['attention_mask'].to(self.device)
