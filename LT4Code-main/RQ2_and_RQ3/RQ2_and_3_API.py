...
for i in range(len(vocab_tokens)):
    freq_vocab[vocab_tokens[i]] = vocab_samples[i] / total_smaple
...
all_apis_each_smaple_scores = []
for l in all_apis:
    score_ = 0
    for api in l:
        score_ += 1 / (freq_vocab[api])
    score_ = score_ / math.sqrt(len(l))
    all_apis_each_smaple_scores.append(score_)
print(len(all_apis_each_smaple_scores), all_apis_each_smaple_scores[0:10])
threshold = np.quantile(all_apis_each_smaple_scores, .50)
...
def process_results_for_plot(ref_path, pred_path, vocab_tokens, freq_vocab, model_name=None):
    references = open(ref_path, 'r').readlines()
    candidates = open(pred_path, 'r').readlines()
...
def Head_Tail_Sets_Results(ref_path, pred_path, vocab_tokens, freq_vocab, threshold, model_name=None):
    ...
    em_ = EM_samples(references, candidates)
    print('EM:', em_)
    #calculate_blue_samples(references, candidates, '2')
    print('HEAD:', len(head_preds))
    em_ = EM_samples(head_labels, head_preds)
    print('EM:', em_) 
    #calculate_blue_samples(head_labels, head_preds, '2')
    print('TAIL:', len(tail_preds))
    em_ = EM_samples(tail_labels, tail_preds)
    print('EM:', em_) 
    #calculate_blue_samples(tail_labels, tail_preds, '2')



"""
#
metrics_by_ratios1 = process_results_for_plot("../generated_predictions/api_rec/Results/MulaRec/CE/test_ref.csv", \
                                              "../generated_predictions/api_rec/Results/MulaRec/CE/test_hyp.csv", vocab_tokens, freq_vocab)


metrics_by_ratios2 = process_results_for_plot("../generated_predictions/api_rec/Results/CodeT5/CE/test.gold", \
                                             "../generated_predictions/api_rec/Results/CodeT5/CE/test.output", vocab_tokens, freq_vocab, 'codet5')


metrics_by_ratios3 = process_results_for_plot("../generated_predictions/api_rec/Results/CodeBERT/CE/test.gold", \
                                             "../generated_predictions/api_rec/Results/CodeBERT/CE/test.output", vocab_tokens, freq_vocab)
...
series = pd.DataFrame({'MulaRec':metrics_by_ratios1, 'CodeT5':metrics_by_ratios2, 'CodeBERT': metrics_by_ratios3, 'x':list(range(1,11,1))})
...
Head_Tail_Sets_Results("../generated_predictions/api_rec/Results/MulaRec/CE/test_ref.csv", \
                       "../generated_predictions/api_rec/Results/MulaRec/CE/test_hyp.csv",
                       vocab_tokens, freq_vocab, threshold)


Head_Tail_Sets_Results(
     "../generated_predictions/api_rec/Results/CodeT5/CE/test.gold", \
     "../generated_predictions/api_rec/Results/CodeT5/CE/test.output",
     vocab_tokens, freq_vocab, threshold, 'codet5')


Head_Tail_Sets_Results(
     "../generated_predictions/api_rec/Results/CodeBERT/CE/test.gold", \
     "../generated_predictions/api_rec/Results/CodeBERT/CE/test.output",
     vocab_tokens, freq_vocab, threshold)
